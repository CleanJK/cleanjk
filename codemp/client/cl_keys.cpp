...
const keyname_t keynames[MAX_KEYS] = {
...
void Message_Key( int key ) {
	...
	if ( key == A_ENTER || key == A_KP_ENTER ) {
		const char *chatInput = chatField.GetLine();
		if ( chatInput[0] && cls.state == CA_ACTIVE ) {
				 if ( chat_playerNum != -1 )	Com_sprintf( buffer, sizeof( buffer ), "tell %i \"%s\"\n", chat_playerNum, chatInput );
...
void CL_CharEvent( int key ) {
	...
	// distribute the key down event to the appropriate handler
		 if ( Key_GetCatcher() & KEYCATCH_CONSOLE ) { Console_CharEvent( key ); }
	...
	else if ( Key_GetCatcher() & KEYCATCH_MESSAGE ) { chatField.CharEvent( key ); }
	else if ( cls.state == CA_DISCONNECTED ) {        Console_CharEvent( key ); }
}
