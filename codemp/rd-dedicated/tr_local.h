...
// a trMiniRefEntity_t has all the information passed in by the client game, other info will come from it's parent main ref entity
...
struct shader_t {
	...
	int            contentFlags;
	bool           defaultShader; // we want to return index 0 if the shader failed to load for some reason, but R_FindShader should still keep a name allocated for it, so if something calls RE_RegisterShader again with the same name, we don't try looking for it again
